...
- **Playwright's *headless* Chromium reports `navigator.gpu` as available but then fails adapter negotiation** (`[warning] No available adapters.`) — `WebGPURenderer` correctly falls back to WebGL2 and logs `THREE.WebGPURenderer: WebGPU is not available, running under WebGL2 backend.`. This is a headless-mode limitation, not a hardware one: this host does have a real GPU, and **running the exact same driver with `HEADED=1` (a real, visible browser window) gets a real `renderer: WebGPU` adapter**, confirmed directly (`HEADED=1 node .claude/skills/run-particles-simulator/driver.mjs ...`). Use plain headless for anything CPU-backend/visual/UI-only (faster, no window pops up); use `HEADED=1` specifically when a check needs to exercise `GpuBackend`'s actual compute kernels rather than just the WebGL2-fallback render path.
...
- **Real-time `wait <ms>` + one radius/energy check can look like a bug when it's actually just bad timing.** With gravity dynamics evolving fast (collapse-and-rebound can complete in ~1 real second once N or G is large enough), a single `wait N; eval <check>` sample can land after the interesting transient already happened, making correct physics look wrong (e.g. RMS radius reading *larger* than the initial value, which looks like repulsion but is really just the post-rebound virialized state). Prefer pausing (`window.__debug.app.playing = false`), then driving the sim with manual fixed-dt `window.__debug.app.getBackend().step(dt)` calls in a single `eval` loop and collecting a full trace array in one shot -- deterministic, not dependent on this machine's real frame rate, and shows the whole trajectory instead of one ambiguous sample.
...
- **Grid-based force approximations have (at least) two distinct, easy-to-conflate failure modes, and fixing one can cause the other.** `computeGridGravity`'s far-field term went through both: (1) using each particle's *exact* position against a distant cell's monopole isn't momentum-conserving (two particles in the same cell don't pull equally hard on a distant cell, so the reaction back doesn't balance, compounding every step into unbounded energy injection -- invisible over a 4-second trace, obvious as steady RMS-radius growth over 60+). (2) The seemingly-obvious fix -- use the particle's *own coarse cell's center of mass* instead, which is exactly momentum-conserving between any two cells by construction -- trades that bug for a different, also-real one: every particle sharing a coarse cell now gets an *identical* far-field vector, so the cloud visibly fragments into one clump per coarse cell (worst at the grid's corner cells, since those have the most lopsided neighbor distribution -- reported by the user as "8 clumps in the corners of a cube," reproduced directly by screenshotting ~1-4 simulated seconds in). The actual fix keeps exact-position far-field (smooth per-particle variation, no fragmentation) and separately cancels the resulting net momentum bias with a cheap O(N) global correction (sum of m*a, then subtract per unit mass) -- fixes the conservation problem without touching how individual particles' forces vary with position. **Bug (2) is exactly the kind of thing that looks like an improvement in a quick check** (heating fixed!) but needs a real screenshot a few seconds into playback to catch, not just a numeric energy trace -- both failure modes are worth checking after touching this function, visually *and* numerically. **This matters again for the M6 GPU port**: the global correction is a "reduce then broadcast-subtract" pattern, the same shape a compute shader reduction already needs, not something to drop for being CPU-loop-shaped.
...
- **`typescript@7.x`'s new native Go-based compiler (`typescript-go`) can hang for 5-10 minutes and then crash with an out-of-memory fatal error when type-checking TSL's heavily-chained/proxied node API** (confirmed via the crash's own goroutine stack trace: deep recursive `checker.(*Relater).recursiveTypeRelatedTo` chains hundreds of frames deep, inside `github.com/microsoft/typescript-go/internal/checker`). This first showed up in M5 (`src/sim/gpu/GpuBackend.ts`) — the CPU backend never touches TSL types at all, so nothing before M5 exercised this. **Fix: `typescript` is pinned to `^6.0.3` (the last pre-native-rewrite major line) in `package.json` — do not upgrade to 7.x while this project uses TSL**, until/unless a future TypeScript 7 release fixes this. If `tsc -b` ever seems to hang again after a `typescript` version bump, this is the first thing to suspect. Also prefer calling `./node_modules/.bin/tsc.cmd` directly over `npx tsc` in this repo/environment — `npx tsc --version` alone was independently observed to hang, seemingly unrelated overhead on top of the above.
- **`window.__debug` is a dev-only hook** (`src/main.ts`, gated on `import.meta.env.DEV`) exposing `{ app, params, monitors }` for numeric verification via `eval`. Since M5 (GPU/CPU backend switching), there's no single persistent `backend` variable to expose directly -- use `window.__debug.app.getBackend()` to reach whichever backend is currently active (CPU or GPU), e.g. `eval window.__debug.app.getBackend().getSnapshot().positions[0]` for CPU, or `eval await window.__debug.app.getBackend().debugReadPositions()` for GPU (async, and only meaningful with a real WebGPU adapter). `eval window.__debug.app.playing` / `eval window.__debug.app.timeScale = 5` still work the same as before. Prefer this over screenshots whenever the thing being verified is numeric (physics correctness) rather than visual.
