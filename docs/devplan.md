...
- **Performance at this milestone's actual target scale, now measurable** via a new debug-only `window.__debug.debugRebuild(count, gpu)` hook that bypasses `PARTICLE_COUNT_MAX` (the Tweakpane slider's cap, never enforced below the UI) — real GPU hardware, one step measured via wall-clock time with a forced sync (`debugReadPositions()`) after a batch of steps to avoid measuring only command-buffer enqueue time: **~41ms/step at N=100,000**, **~294ms/step at N=250,000**. The 100k case is comfortably interactive; 250k is not (~3.4fps) — consistent with the already-documented risk that the two genuinely-serial single-thread passes (`GpuUniformGrid`'s prefix sum, `GpuBackend`'s momentum reduction) dominate at large N. Profiling which of the two is the actual bottleneck, and a parallel (Hillis-Steele/Blelloch) replacement for one or both, is the natural next optimization — not built upfront per this project's stated priorities, but no longer a documented future concern, now a documented *measured* one.
...
Not yet built (per the milestone list below): `stats/`, `bench/`, `ui/StatsReadout.ts`, `render/gizmos.ts`, `render/postfx.ts`, `render/colorRamp.ts`.

## UI / controls (Tweakpane) — as built so far

Performance (FPS graph, CPU%/GPU% frame-time estimate), Simulation (Playing, Speed, Restart, **Use GPU** — manual toggle, no adaptive benchmarking/graying-out yet), Gravity (G, Softening — both live, apply to whichever backend is active as of M6), Point Cloud (N, Box Radius — both live; slider still capped at `PARTICLE_COUNT_MAX=20,000` for both backends, unaffected by M6 — raising it for GPU-scale N is M7's job). Still to come per the plan: restitution `e` + merge-on-contact toggle, radiation-rate constant, species mix ratio + per-species visibility, stats-overlay toggle, bloom strength/threshold.
...
7. **M6** ✅ — Port grid-gravity to TSL at N=100k–250k on GPU. As built: a from-scratch TSL counting-sort + near/far-field gravity port (no local reference example existed to match against, unlike M5) -- type-checks and pattern-matches confirmed TSL API usage, but has not executed even once in this environment (no real WebGPU adapter). See As-built deviations above; needs numeric verification against `computeGridGravity` on real GPU hardware before trusting it.
