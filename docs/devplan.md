...
- **Still not verified**: exact numeric parity against `computeGridGravity`'s CPU output (would need matching initial positions across backends, which isn't currently wired up — CPU uses `Math.random()`-based Box-Muller, GPU uses `hash(instanceIndex)`-based, so today's check is statistical/behavioral, not a direct per-particle diff). Also not yet checked: larger N (100k–250k, this milestone's actual target scale) or GPU frame-time/performance, since raising `PARTICLE_COUNT_MAX` past 20,000 is explicitly M7's job and there's no debug hook yet to bypass the UI slider for a one-off larger-N GPU test. Worth doing both before relying on this at production scale.
...
- **The GPU path can now be checked on real hardware from this environment** — headless Chromium fails WebGPU adapter negotiation (M0/M5's original caveat), but a headed browser window on the same machine gets a real adapter (`HEADED=1` in `driver.mjs`, see M6 as-built deviations). M6's gravity kernels were spot-checked this way (N=5,000: correct collapse-and-rebound, no NaNs, no fragmentation) — real, but still only one N, one param set, and no numeric parity check against the CPU reference. Larger N (this milestone's actual 100k–250k target) and a direct `computeGridGravity` comparison are still open; `GpuParticlePoints`'s render path specifically hasn't been re-confirmed since M5.
- **Two serial single-thread GPU passes** (`GpuUniformGrid`'s prefix sum, `GpuBackend`'s momentum reduction — both M6) are an accepted-for-now performance bottleneck, mirroring the CPU grid's already-serial prefix sum above. Likely the dominant per-step GPU cost at large N; a parallel (Hillis-Steele/Blelloch) version of both is the natural M7-or-later optimization if profiling on real hardware confirms it.
